...
class AiService {
   ...
   * 批量生成AI解读
   */
  async batchGenerateInterpretation(data: BatchInterpretationRequest): Promise<BatchInterpretationResult> {
    try {
      const response = await request({
   ...
   * 重新生成解读
   */
  async regenerateInterpretation(interpretationId: string, options?: InterpretationRequest['options']): Promise<InterpretationResult> {
    try {
      const response = await request({
   ...
   * 优化解读结果
   */
  async optimizeInterpretation(interpretationId: string, strategy?: string): Promise<OptimizationResult> {
    try {
      const response = await request({
   ...
   * 获取支持的占卜类型
   */
  async getSupportedTypes(): Promise<Array<{
    type: string
    name: string
    description: string
    enabled: boolean
   ...
   * 提交用户反馈
   */
  async submitFeedback(interpretationId: string, feedback: {
    rating: number
    comment?: string
    helpful: boolean
    tags?: string[]
