...
export const batchRequest = async <T = any>(
  ...
  return results.map(result => (result instanceof Error ? result : result.data));
};

// 创建专用请求方法
export const createCachedRequest = <T = any>(url: string, ttl: number = 5 * 60 * 1000) => {
  return requestUtils.createCachedGetter<T>(url, ttl);
};

export const createRetryRequest = <T = any>(config: RequestOptions, retryTimes: number = 3) => {
