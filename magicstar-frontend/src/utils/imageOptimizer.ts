...
export class ImageOptimizer {
  ...
  // 压缩图片
  async compressImage(filePath: string, config: ImageCompressConfig): Promise<string> {
    try {
      const {
        quality = 0.8,
        maxWidth = 1920,
  ...
  async preloadImages(urls: string[], config?: Partial<PreloadConfig>): Promise<void> {
    ...
    const urlsToLoad = urls.filter(
      url => !this.preloadedImages.has(url) && !this.loadingImages.has(url)
    );
...
export function useImageCompress() {
  ...
  const generateResponsiveUrl = (baseUrl: string, width: number, quality: number = 80): string => {
...
export const imageUtils = {
  ...
  // 生成缩略图URL
  generateThumbnailUrl: (originalUrl: string, size: number = 200, quality: number = 70): string => {
    const optimizer = ImageOptimizer.getInstance();
