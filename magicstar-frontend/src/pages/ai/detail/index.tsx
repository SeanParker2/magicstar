...
import { aiService, InterpretationResult, QualityAssessment, OptimizationResult } from '../../../services/ai'
import './index.scss'
  ...
  const loadInterpretationDetail = async () => {
    try {
      ...
      const mockInterpretation: InterpretationResult = {
        id: id || '1',
        type: 'tarot',
        content: '根据您抽取的塔罗牌，我看到了一个充满变化和机遇的时期正在到来。愚者牌代表新的开始和无限的可能性，它提醒您要保持开放的心态，勇敢地踏出第一步。星币三代表合作和技能的发展，暗示您在工作或学习方面将会有新的进展。宝剑七则警示您要小心处理人际关系中的冲突，避免因为误解而产生不必要的纷争。',
        summary: '新的开始即将到来，保持开放心态，注意人际关系处理',
        advice: '建议您在接下来的时间里，积极拥抱变化，同时保持谨慎的态度。在做重要决定时，多听取他人的意见，但最终要相信自己的直觉。',
        qualityScore: 88,
        metadata: {
          ...
          <View className='metadata-section'>
            ...
            <View className='metadata-item'>
              ...
              <Text className='metadata-value'>
                ...
                (提示: {interpretation.metadata.tokenUsage.promptTokens}, 
                完成: {interpretation.metadata.tokenUsage.completionTokens})
              </Text>
            </View>
            <View className='metadata-item'>
            ...
            <View className='metadata-item full-width'>
              ...
              <Text className='metadata-value prompt'>
                {interpretation.metadata.promptUsed}
              </Text>
            </View>
          </View>
        )}
            ...
            {optimization.improvements.length > 0 && (
              <View className='improvements'>
                ...
                {optimization.improvements.map((improvement, index) => (
                  <Text key={index} className='improvement-item'>• {improvement}</Text>
                ))}
              </View>
            )}
          </View>
